...
class SeDIDLikeMapDataset(Dataset):
    """
    Dataset for precomputed SeDID-like maps / noise profiles.
    ...
    The map can be:
        - a .pt tensor of shape [C, H, W]
        - an image file such as .png or .jpg
    """
    ...
    def __init__(
        ...
        if len(self.records) == 0:
            raise RuntimeError(
                f"No SeDID-like maps found in {self.root_dir}. "
    ...
    def _fix_channels(self, x: torch.Tensor) -> torch.Tensor:
        """
        Convert map channels to the expected number of channels.
        """
        ...
        if x.shape[0] == 3 and self.in_channels == 1:
            return x.mean(dim=0, keepdim=True)

        if x.shape[0] == 4 and self.in_channels == 3:
            # If the map comes from a latent representation with 4 channels,
            # reduce it to 3 channels for a standard ResNet input.
            return x[:3]

        if x.shape[0] == 4 and self.in_channels == 1:
            return x.mean(dim=0, keepdim=True)

        raise ValueError(
            f"Cannot convert map with {x.shape[0]} channels "
            f"to expected {self.in_channels} channels."
        )
